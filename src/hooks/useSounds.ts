...
export function useSounds() {
  const sounds = useMemo(() => {
    ...
    return {
      bgm: new Howl({
        ...
        onloaderror: (id, err) => console.error('Erro ao carregar BGM:', err),
      }),
      gameOver: new Howl({
        src: ['/assets/sfx/game-over.mp3'],
        volume: 0.8,
        onload: () => console.log('Game Over som carregado'),
        onloaderror: (id, err) => console.error('Erro ao carregar Game Over:', err),
      }),
      heartbeat: new Howl({
        ...
        onloaderror: (id, err) => console.error('Erro ao carregar Heartbeat:', err),
      }),
      newPursuer: new Howl({
        src: ['/assets/sfx/new-pursuer.mp3'],
        volume: 0.6,
        onload: () => console.log('New Pursuer som carregado'),
        onloaderror: (id, err) => console.error('Erro ao carregar New Pursuer:', err),
      }),
      scoreBonus: new Howl({
        src: ['/assets/sfx/score-powerup.mp3'],
        volume: 0.7,
        onload: () => console.log('Score Bonus som carregado'),
        onloaderror: (id, err) => console.error('Erro ao carregar Score Bonus:', err),
      }),
      multiplierBonus: new Howl({
        src: ['/assets/sfx/multiplier-powerup.mp3'],
        volume: 0.7,
        onload: () => console.log('Multiplier Bonus som carregado'),
        onloaderror: (id, err) => console.error('Erro ao carregar Multiplier Bonus:', err),
      })
    };
  }, []);
  ...
  const playSound = useCallback((soundName: keyof typeof sounds) => {
    ...
    // Não para todos os sons, apenas toca o solicitado
